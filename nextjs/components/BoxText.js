...
const BoxContainer = () => {
  ...
  const [currentBoxSet, setCurrentBoxSet] = useState(0);
  const [screenWidth, setScreenWidth] = useState(1024); // Default value
  ...
  useEffect(() => {
    // Update screenWidth only if JavaScript is enabled
  ...
  // Calculate adjustedCurrentBoxSet, startIndex, and endIndex as before...

  return (
    <div className="box_container">
      {/* Box Controls */}
      {/* For simplicity, I'll omit the buttons' disabled states in this example */}
      <div className="box_controls">
        <div onClick={handlePreviousBoxSet} className={`previous-button ${currentBoxSet === 0 ? "disabled" : ""}`}>
          <span>&#10094;</span>
        </div>
      </div>
